...
class WikipediaParser(BaseParser):
    ...
    WIKI_TARGETS :  List [str] = ['.mw-parser-output', '.mw.content-ltr']

    WIKI_EXCLUSIONS : str = '''
            .infobox, .sinottico, .mw-editsection, .mw-references-wrap, .mw-references-columns, .CdA, .mw-empty-elt, 
            .hatnote, .avviso, .avviso-contenuto, .vedi-anche, .mw-file-description, .mw-file-element, .navigation-not-searchable, 
            ...
            .navbox, .vertical-navbox, .side-box, .printfooter, .metadata, .catlinks, table.sidebar, table.infobox, .awards-table, .accolades-table, 
            .mw-references-wrap, .mw-references-columns, img.skin-invert, .mv-hiero-table, div.navbox, table.plainlinks, .reflist
    '''
    WIKI_MARKDOWN_OPTIONS : Dict[str, bool]= { 
        'ignore_images': True, 
        'escape_html': True,
        'ignore_links': True
    }

    WIKI_MARKDOWN_INUTILE :  List [str] = [
        "## See also", "## Notes", "## References", "## External links", "## Bibliography", 
        "## Further readings", "## Explanatory notes", "## Notes and references"
    ]
    
    WIKI_GEN_EXCLUDED_TAGS : List[str] = ['nav', 'script', 'style', 'img', 'noscript', 'figure', 'meta', 'cite', 'link', 'audio']

    WIKI_MODE : CacheMode = CacheMode.BYPASS

    def __init__(self):
        super().__init__(
            targets = self.WIKI_TARGETS,
            css_exclusions = self.WIKI_EXCLUSIONS,
            gen_excluded_tags = self.WIKI_GEN_EXCLUDED_TAGS,
            general_markdown_options = self.WIKI_MARKDOWN_OPTIONS,
            markdown_inutile = self.WIKI_MARKDOWN_INUTILE,
            mode = self.WIKI_MODE
    )
        self.html_local : str = ""
    ...
    def clean_text(self, markdown_text: str) -> str:
        ...
        markdown_text = re.sub(r'\[\d+\]\(#cite_note-[^)]+\)', '', markdown_text)
        markdown_text = re.sub(r'\[\[[0-9]+\]\]', ' ', markdown_text)
    ...
    def clean_raw_html(self, raw_html: str) -> str:
        ...
        raw_html = re.sub(r'<div class="navbox".*?>.*?</div>', '', raw_html, flags=re.DOTALL | re.IGNORECASE)
        raw_html= re.sub(r'<div class="catlinks".*?>.*?</div>', '', raw_html, flags=re.DOTALL | re.IGNORECASE)
        ...
        raw_html = re.sub(r'<noscript.*?>.*?</noscript>', '', raw_html, flags=re.DOTALL | re.IGNORECASE)
        raw_html = re.sub(r'<footer.*?>.*?</footer>', '', raw_html, flags=re.DOTALL | re.IGNORECASE)
        raw_html = re.sub(r'<[a-z0-9]+ class="mw-empty-elt".*?></[a-z0-9]+>', '', raw_html)
        raw_html = re.sub(r'<span[^>]*class="cite-bracket"[^>]*>.*?</span>', '', raw_html)
        return raw_html
    ...
    def extract_title(self, soup: BeautifulSoup):
        ...
        elem_titolo = soup.find('h1', id='firstHeading') #se trova il tag html h1 usa questo, altrimenti

        title : str = ""
        if elem_titolo:
            title= elem_titolo.get_text(strip=True) #.get_text estrae solo il testo che sta nei tag html <title>, strip=True rimuove spazi, \n o \t
        else:
    ...
    async def parse_url(self, url: str) -> Dict[str, str] | None:
        ...
        if self.html_local:
            ...
            cleaned : str = self.clean_raw_html(self.html_local)
            target_to_crawl = f"raw:{cleaned}"
